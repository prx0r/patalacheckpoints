...
class DerivedScholarlyObject:
    ...
    def derive_ceiling(self) -> str:
        """R3: epistemic_ceiling = derive(authority vector). The review axis is BINDING and is the
        sole upward path (G3 R1): a human review/adjudication sets the ceiling regardless of machine
        evidence, because only a human can grant INDEPENDENT_REVIEWED/ADJUDICATED."""
        rev = self._axis("review")
        if rev > 0:  # a human has reviewed/adjudicated → that is the ceiling
            for label, r in sorted(EPISTEMIC_RANK.items(), key=lambda kv: kv[1]):
                if r == rev:
                    return label
            return "INDEPENDENT_REVIEWED"
