...
5. **`docs/vision/atlas/atlas-engineering-blueprint.md`** + **`atlas-cloudflare-edge-layer.md`** + **`atlas-performance.md`** + **`agent-optimization.md`** (the agent product surface: discoverability/usability/execution, 7 MCP tools, 2 MCP surfaces, token budgets, agent evals) + **`docs/atlas-contracts/access-policy.md`** (public discovery ≠ public corpus: tiered API public/research/bulk, AI-crawler policy, robots.txt is NOT a security boundary) — the build blueprint, the Cloudflare edge layer, and the performance doctrine ("compute on write, not read; exact versions are static; one agent question = one request; Astro islands; Rust only for hot kernels").
