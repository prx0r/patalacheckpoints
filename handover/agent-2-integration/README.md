...
| **`docs/vision/atlas/technical-architecture-v1.md`** | **The authoritative Technical Architecture v1** — the Pāṭala Authority Graph, the full SQL schema + Pydantic epistemic objects, the 3 P0 schema corrections (no loose `dict` content; no single scalar authority rank; no universal review ladder), and the exact locked stack. **Read before writing schemas.** |
...
| **`docs/vision/atlas/agent-optimization.md`** | **Agent optimization as its own product surface** — discoverability (SEO/HTML/sitemaps/JSON-LD/llms.txt), usability (7 MCP tools with ontology-teaching descriptions, 2 MCP surfaces: public read-only + scholar OAuth), execution (compiled R2 bundles, token budgets, one-question-one-call, agent evals). The agent moat: "an agent dealing with Sanskrit textual scholarship should not have to rediscover what work/edition/passage/argument it is looking at — it resolves it through Pāṭala." |
| **`docs/atlas-contracts/access-policy.md`** | **Public discovery ≠ public corpus** — tiered access (public/research/bulk API + public/research MCP), AI-crawler policy (block training, allow search/user agents), robots.txt is NOT a security boundary (protect moat data with auth/WAF/object perms). |
| **`docs/atlas-contracts/`** | **OpenAlex-level reference docs for the built contracts** — `overview.md` (the model + 3 P0 fixes), `ids.md` (stable vs version identity), `authority-vector.md` (the 4-axis AuthorityVector), `objects.md` (the typed epistemic objects), `atlas-database.md` (the 22-table Authority Graph schema). |
