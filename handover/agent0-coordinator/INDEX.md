...
- **`handover/agent0-coordinator/hermespatala.md`** — **THE canonical Hermes×Pāṭala integration reference** (merges the backend model + the advanced recipes): the mental separation (Pāṭala = epistemic state machine, Hermes = cognitive execution fabric), the feature→vision map, the 16 advanced recipes (adversarial subagents, blind adjudication tournaments, execute_code, graph-as-verbs plugin, capability-scoped MCP, hooks-as-triggers, graph-aware review scheduling, scholarly CI, trajectory-as-dataset, counterfactual swarms, self-improving-but-gated skills), and the three integrations to build first (skill pack, MCP capability layer, blind adversarial review). Source: `hermespatala-advanced-recipes-source.md`.
