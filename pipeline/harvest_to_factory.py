...
def _extract_muktabodha_text(path: Path) -> list[str]:
    """Muktabodha IAST: verse lines end with '|| N ||' (numbered verses) with prose interleaved."""
    verses = []
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        line = line.strip()
        # a verse line ends in a numbered marker like '|| 1 ||' and has IAST diacritics
        if line.endswith("||") and re.search(r"\|\|\s*\d+\s*\|\|", line):
            verse = re.sub(r"\s*\|\|\s*\d+\s*\|\|\s*$", "", line).strip()
...
def write_work_jsonl(work_id: str, verses: list[str], source: str, dry_run: bool = False) -> int:
    ...
    for i, v in enumerate(verses):
        ...
        if not v:
            continue
        rows.append({
            "work": work_id, "verse_idx": i + 1,
            "source_sha256": hashlib.sha256(v.encode("utf-8")).hexdigest(),
            "sanskrit": v, "translation": "", "status": "pending",
